...
- Modify: `apps/macos/macos/Jujutsushi.xcodeproj/project.pbxproj`
...
In `apps/macos/macos/Jujutsushi.xcodeproj/project.pbxproj`, find **both** occurrences of `MACOSX_DEPLOYMENT_TARGET` (one in the Debug build configuration, one in Release, both under the `Jujutsushi-macOS` target — not any leftover unused `-iOS` config block the shared template carries) and change:
...
Run: `cd apps/macos && npx react-native build-macos && cd ../..`
Expected: exits 0. If it fails with a Swift/bridging-header-related error, re-check Step 4's build settings landed correctly (`grep SWIFT_OBJC_BRIDGING_HEADER apps/macos/macos/Jujutsushi.xcodeproj/project.pbxproj` should show the path set in Step 4).
...
git add apps/macos/macos/Jujutsushi-macOS/Jujutsushi-macOS-Bridging-Header.h \
        ...
        apps/macos/macos/Jujutsushi.xcodeproj/project.pbxproj \
